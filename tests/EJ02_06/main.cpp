...
uint32_t createVertexData(uint32_t *VBO, uint32_t *EBO, float radius) {
	float vertices[(6 + 1) * 3];
	...
	for (int i = 1; i <= 6; i++) {
		float angle = 60.0f + (i - 1) * 60.0f;

		vertices[(i * 3)] = radius * glm::cos(glm::radians(angle));
		vertices[(i * 3) + 1] = radius * glm::sin(glm::radians(angle));
		vertices[(i * 3) + 2] = 0.0f;
	}
...
int main(int argc, char* argv[]) {
	...
	uint32_t VBO, EBO;
	uint32_t VAO = createVertexData(&VBO, &EBO, 0.5f);
